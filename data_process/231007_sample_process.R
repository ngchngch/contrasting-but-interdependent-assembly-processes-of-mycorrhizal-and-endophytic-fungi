...
##
save.dir <- "analysis/230414_sample_process"

dir.create(save.dir)

#set param
vsearchpath="/usr/local/bin/vsearch"
minident <- c(0.97,0.93)

###read sequence files
#$Fungi

seqfiles <- list.files("Raw_data/all_seqtab/Fungi",full.names = T)
...
# otu clustering
  ...
  system2(command = vsearchpath, 
          args = c("--cluster_fast $input", sprintf("%s/%s/merge_seq.fasta", save.dir,exp),
                   "--id", minident,
                   "--mothur_shared_out", sprintf("%s/%s/ASV_OTU_corestab_%s.txt", save.dir,exp, minident[2]),
                   "--centroids", sprintf("%s/%s/OTUseq_%s.fasta", save.dir,exp, minident[2]),
                   "--msaout",  sprintf("%s/%s/seqAlign_%s.txt", save.dir,exp, minident[2]) ) )
  
  otu <- read.table(sprintf("%s/%s/ASV_OTU_corestab_%s.txt", save.dir,exp, minident[2]),
                    header=TRUE, row.names=2)[,-c(1:2)]
  ...
  saveRDS(otutab, sprintf("%s/%s/seqOTUtab_%s.rds", save.dir,exp,minident[2]))
  write.csv(cbind(sample=rownames(otutab), otutab), sprintf('%s/%s/seqOTUtab_%s.csv',save.dir,exp,minident[2]), row.names=FALSE)
  ...
  seqfiles <- list.files("Raw_data/all_seqtab/Plant",full.names = T)
