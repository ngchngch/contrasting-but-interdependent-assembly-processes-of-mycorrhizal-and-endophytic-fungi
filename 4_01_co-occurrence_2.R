...
slr <- spiec.easi(input, method='slr', r=0:20, lambda.min.ratio=1e-3,
...
##
otu_conv <- read.table("bioinfo_output/230523_makeOTUtables/0.93/ASV_OTU_corestab_0.93.txt",
                       header=T,row.names=2)

gconv <- gather(cbind(ASV=rownames(otu_conv),as.data.frame(otu_conv[,-c(1,2)])),OTU,tf,-1)
gconv2 <- gconv[gconv$tf==1,]
rownames(gconv2) <- gconv2[,1]

raw_df2 <- Taxa.mat(raw_df,gconv2[colnames(raw_df),],"OTU")
