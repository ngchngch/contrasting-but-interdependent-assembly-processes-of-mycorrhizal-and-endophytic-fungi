...
save.dir <- sprintf("%s/02_jsdm_model_comp_community_level_OTU93",current_dir)
...
######
##########################
ll <- readRDS(sprintf("%s/jsdm_result_in_google_colab/OTU_0.93/restricted_lls_mat_OTU93.rds",current_dir))

a <- readRDS(sprintf("%s/jsdm_result_in_google_colab/OTU_0.93/jsdm_sdist_result_iter100_Msamp5000_OTU93.rds",current_dir))

mod <- readRDS(sprintf("%s/jsdm_result_in_google_colab/OTU_0.93/pred_rawdata_OTU93.rds",current_dir))
############################

prab <- a$data$Y

modf <- mod[,c(2,4)]
colnames(modf) <- c("pres","prob")
ROC <- pROC::roc(pres ~ prob,
                 data = modf,
                 ci=TRUE)

png(sprintf("%s/All_ROC_OTU93.png",save.dir),width = 2000,height = 2000,res=300)
plot(ROC,　# roc()で作成したオブジェクト
     ...
     main="Cutoff sequence similarity = 93%",
